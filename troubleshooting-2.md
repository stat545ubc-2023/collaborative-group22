...
#Correction: "dslabs" package installed before loading.

install.packages("dslabs", repos = "http://cran.us.r-project.org")
```

    ## Installing package into 'C:/Users/igsuw/AppData/Local/R/win-library/4.3'
    ## (as 'lib' is unspecified)

    ## package 'dslabs' successfully unpacked and MD5 sums checked
    ## 
    ## The downloaded binary packages are in
    ##  C:\Users\igsuw\AppData\Local\Temp\Rtmpol5WV7\downloaded_packages

``` r
#Correction: Syntax to load libraries was modified.
...
#Correction: CRAN Mirror repo link was added.
...
``` r
#Correction: dim() was replaced by glimpse() to have a look at the tibble
...
``` r
#Correction: conditional operator was changed from >= to >.
...
``` r
#Correction: expression 'month == 2000' was changed to 'year == 2000'
...
``` r
### JUSTIN FIXED ERROR ###
#Correction: use everything() function to represent everything else 
movieLens %>%
  filter(!genres == "Drama") %>%
...
``` r
### JUSTIN FIXED ERROR  ###
##Correction: two equal sign mean evalute the left side is qeual to right side and output a boolean value
movieLens %>%
...
```
    ...
    ##  9     1339 Dracula (Bram Stoker's Dracul…  1992 Fanta…       1    3.5    1.26e9
...
``` r
### JUSTIN FIXED ERROR  ###
#Correction: mutate() function can drop all others column
movielens %>%
...
``` r
### JUSTIN FIXED ERROR  ###
#Correction: count() function is a shorthand for group_by() and tally()
movieLens %>%
  count(year)
```
...
``` r
###JUSTIN FIXED ERROR  ###
#Correction: dont need the c() function to form a vector 
movieLens %>%
...
```
    ...
    ##    title                              min_rating
    ##    <chr>                                   <dbl>
    ##  1 "\"Great Performances\" Cats"             0.5
    ##  2 "$9.99"                                   2.5
    ##  3 "'Hellboy': The Seeds of Creation"        2  
    ##  4 "'Neath the Arizona Skies"                0.5
    ##  5 "'Round Midnight"                         0.5
    ##  6 "'Salem's Lot"                            3.5
    ##  7 "'Til There Was You"                      0.5
    ##  8 "'burbs, The"                             1.5
    ##  9 "'night Mother"                           5  
    ## 10 "(500) Days of Summer"                    0.5
    ## # ℹ 8,822 more rows
